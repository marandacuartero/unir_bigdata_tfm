...
public class MongoDBWriter implements IMongoDBWriter {
	...
	public long cleanFrom(Date fecha) {
		...
		try (MongoClient mongoClient = MongoClients.create(connectionString)) {
			...
			collection.createIndex(Indexes.descending("TTFecha"));
			Document filter = new Document("TTFecha", new Document("$gte", fecha));
	...
	private Document saveTrazas(TrazaTorneo tt) {
		...
		document.append("TTtotalPuntos", tt.getTotalPuntos());
		document.append("TTtotalDieces", tt.getTotalDieces());
		document.append("TTtotalEquis", tt.getTotalEquis());
		return document;
	}
